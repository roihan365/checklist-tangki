    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
            ...
            <div class="flex space-x-3">
                <x-dashboard.button href="{{ route('distributor.checklist-schedule.index') }}" variant="secondary">
                ...
                <div
                    ...
                    <div class="p-5">
                        ...
                        <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Hasil Checklist
                                Terakhir</h4>

                            @if ($schedule->checklistDocuments->count() > 0)
                                @php
                                    $latestDocument = $schedule->checklistDocuments->sortByDesc('created_at')->first();
                                @endphp
                                <div class="space-y-3">
                                    <div class="flex items-start">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                                {{ $latestDocument->petugas->nama }}
                                            </p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ $latestDocument->created_at->format('d M Y H:i') }}
                                            </p>
                                            <div class="mt-1">
                                                <a href="{{ Storage::url($latestDocument->file_path) }}"
                                                    target="_blank"
                        ...
                        <div class="mt-6 flex space-x-3">
                            @if ($schedule->status != 'dibatalkan')
                                <x-dashboard.button
                                    href="{{ route('distributor.checklist-schedule.document', [$schedule, $schedule->vehicle]) }}"
